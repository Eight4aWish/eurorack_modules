...
- Features:
	...
	- Patches: Clock, Quant, Euclid, Env (dual envelopes), QuadLFO, Scope, MIDI (USB), Net (network MIDI), Turing, Acid, Diag.
	- **Net patch**: RTP-MIDI / AppleMIDI ("Network MIDI") receiver over the Pico 2 W's WiFi — a wireless twin of the USB MIDI patch driving CV0 pitch / CV1 gate / CV2 velocity / CV3 mod. Connect from macOS Audio MIDI Setup (Network) or rtpMIDI on Windows via the module's IP (port 5004). WiFi credentials live in `include/pico2w_oc/secrets.h` (gitignored — copy `secrets.h.example`).
